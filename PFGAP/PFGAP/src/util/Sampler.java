...
//	    Random r = new Random();
	...
	public static ListObjectDataset uniform_sample(ObjectDataset dataset, int n) {
		...
		for (int i = 0; i < n; i++) {
			...
			sample.add(dataset.get_class(i), dataset.get_series(i), dataset._internal_indices_list().get(i));
		}
	...
	public static ListObjectDataset stratified_sample(Map<Integer, ListObjectDataset> data_per_class,
		...
		for (Map.Entry<Integer, ListObjectDataset> entry : data_per_class.entrySet()) {
			...
			for (int i = 0; i < class_sample_size; i++) {
				...
				sample.add(class_sample.get_class(i), class_sample.get_series(i), class_sample._internal_indices_list().get(i));
			}
		}
